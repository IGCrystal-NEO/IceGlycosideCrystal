#!/usr/bin/env python3
"""
igcrystal - Crystal structure analysis and manipulation toolkit (revised)

This file builds on prior iterations with additional robustness and usability:
- element symbol normalization (case-insensitive)
- occupancy support for atoms
- optional per-instance element mass table (override/extend global table)
- safer volume computation and robust lattice vector construction
- density calculation: from (molecular_weight, Z) or auto from per-atom masses
- fractional <-> cartesian conversions using np.linalg.solve for numerical stability
- supercell generation with a safety limit to avoid accidental OOM
- plotting with optional save/show control (avoids blocking in headless env)
- bond plotting: uses scipy.spatial.cKDTree when available for efficiency, falls back to O(N^2)
- JSON IO with validation and format compatibility
- small CLI/demo when run as script
"""
...
# Matplotlib import for plotting (optional at runtime)
...
def extend_element_masses(mapping: Dict[str, float], override: bool = True) -> None:
    ...
    logger.debug("ELEMENT_MASSES extended: %s", list(mapping.keys()))
...
class Crystal:
    ...
    def __str__(self) -> str:
        info = [f"Crystal: {self.name}"]
        lp = self.lattice_parameters
        info.append(f"Lattice parameters: a={lp['a']:.4f} Å, b={lp['b']:.4f} Å, c={lp['c']:.4f} Å")
        info.append(f"Angles: α={lp['alpha']:.4f}°, β={lp['beta']:.4f}°, γ={lp['gamma']:.4f}°")
        info.append(f"Space group: {self.space_group}")
        info.append(f"Number of atoms: {len(self.atoms)}")
        try:
            vol = self.get_volume()
            info.append(f"Unit cell volume: {vol:.6f} Å³")
        except Exception as e:
            info.append(f"Unit cell volume: (error) {e}")
        return "\n".join(info)


# -------------------------
# Helpful factory helpers
# -------------------------
...
# -------------------------
# Module demo / CLI usage
# -------------------------
def main() -> None:
    logger.info("IGCrystal - Crystal Structure Toolkit (revised)")
    logger.info("=" * 60)
    ...
    # Density: option 1 - atomic mass sum (uses ELEMENT_MASSES)
    try:
    ...
    # Density: option 2 - using molecular weight & Z
    carbon_mw = 12.01
    try:
    ...
    # Optional plotting example (save to file rather than show by default)
